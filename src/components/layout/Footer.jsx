...
import { Plane, Github, Twitter, Instagram, Linkedin, Heart } from 'lucide-react'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            <div className="flex items-center gap-3">
              {socials.map(({ Icon, href, label }) => (
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="w-9 h-9 rounded-xl glass flex items-center justify-center text-slate-400 hover:text-white hover:border-brand-500/50 transition-all duration-200"
                >
        ...
        <div className="mt-16 pt-8 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-slate-500 text-sm flex items-center gap-1">
            Made with <Heart className="w-3 h-3 text-rose-500 fill-rose-500" /> by Traveloop Team · {new Date().getFullYear()}
          </p>
