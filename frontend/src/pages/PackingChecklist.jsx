...
export default function PackingChecklist() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (selectedId) {
        ...
        const itemsResult = await tripApi.packingItems(selectedId)
        setItems(itemsResult.data || [])
      } else {
        setItems([])
      }
    } catch (err) {
      setMessage(err.message || 'Unable to load packing list')
    } finally {
      setLoading(false)
    }
  }, [tripId])
  ...
  const toggle = async (id, currentPacked) => {
    setMessage('')
    try {
      await tripApi.updatePackingItem(id, { is_packed: !currentPacked })
      await load()
    } catch (err) {
      setMessage(err.message || 'Unable to update item')
    }
  }
  ...
  const remove = async (id) => {
    setMessage('')
    try {
      await tripApi.removePackingItem(id)
      await load()
    } catch (err) {
  ...
  const add = async () => {
    ...
    setMessage('')
    try {
      await tripApi.createPackingItem(tripId, { item_name: newItem, category: newCategory, is_packed: false })
      setNewItem('')
      await load()
    } catch (err) {
  ...
  const filtered = activeCategory === 'all' ? items : items.filter(i => (i.category || '').toLowerCase() === activeCategory)
  const packedCount = items.filter(i => i.isPacked).length
  const pct = useMemo(() => Math.round((packedCount / (items.length || 1)) * 100), [packedCount, items.length])
      ...
      <div className="p-6 lg:p-8 space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">{packedCount}/{items.length} items packed · {trips.find(t => String(t.id) === tripId)?.title || 'Select a trip'}</p>
          </div>
        ...
        {loading ? (
          <div className="glass-card p-8 text-center text-slate-500">Loading packing items...</div>
        ) : (
          CATEGORIES.filter(c => activeCategory === 'all' || c.id === activeCategory).map(cat => {
            const catItems = filtered.filter(i => (i.category || '').toLowerCase() === cat.id)
                  ...
                  {catItems.map((item, i) => (
                    <motion.div key={item.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                      className={`flex items-center gap-3 p-3 rounded-xl border transition-all group ${
                        item.isPacked ? 'bg-leaf-light border-brand-200' : 'bg-white border-slate-200 hover:border-brand-200'
                      }`}>
                      <button onClick={() => toggle(item.id, item.isPacked)}
                        className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center flex-shrink-0 transition-all ${item.isPacked ? 'bg-emerald-500 border-emerald-500' : 'border-white/20 hover:border-brand-400'}`}>
                        {item.isPacked && <Check className="w-3.5 h-3.5 text-white" strokeWidth={3} />}
                      </button>
                      <span className={`flex-1 text-sm ${item.isPacked ? 'line-through text-slate-400' : 'text-slate-700'}`}>
                        {item.itemName}
                      </span>
