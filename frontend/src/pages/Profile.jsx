  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (active) setMessage(err.message || 'Unable to load profile')
      } finally {
  ...
  return (
    <DashboardLayout>
      <div className="p-6 lg:p-8 space-y-6 max-w-4xl">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-6 relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-center sm:items-start gap-5">
            <div className="relative flex-shrink-0">
              <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-brand-400 to-brand-600 flex items-center justify-center text-white text-3xl font-black">
                {(form.name || user?.firstName || 'T')[0]}
              </div>
            ...
            <div className="flex-1 text-center sm:text-left">
              <h1 className="text-2xl font-bold mb-1" style={{ color: '#1e2d1f' }}>{form.name || 'Traveler'}</h1>
              ...
              <div className="flex gap-4 justify-center sm:justify-start text-sm">
                <div className="text-center">
                  <div className="font-bold text-lg" style={{ color: '#1e2d1f' }}>{user?.trips?.length || 0}</div>
                  <div className="text-slate-500 text-xs">Trips</div>
                </div>
                <div className="w-px bg-white/10" />
                <div className="text-center">
                  <div className="font-bold text-lg" style={{ color: '#1e2d1f' }}>1</div>
        ...
        {loading ? (
          <div className="glass-card p-8 text-center text-slate-500">Loading profile...</div>
        ) : (
