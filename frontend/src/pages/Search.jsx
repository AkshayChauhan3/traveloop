  ...
  return (
    <DashboardLayout>
      <div className="p-6 lg:p-8 space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Globe className="w-6 h-6 text-cyan-400" />Explore Destinations</h1>
          <p className="text-slate-400 text-sm mt-1">Discover cities, activities, and experiences worldwide</p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {REGIONS.map(r => (
            <button key={r} onClick={() => setRegion(r)}
              className={`px-3 py-1.5 rounded-xl text-xs font-medium border transition-all ${region === r ? 'bg-cyan-500/20 border-cyan-500/40 text-cyan-300' : 'glass border-white/10 text-slate-400 hover:text-white'}`}>
              {r}
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-2 border-b border-white/5">
          {['destinations', 'activities'].map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-4 py-2 text-sm font-medium capitalize transition-all ${tab === t ? 'text-white border-b-2 border-brand-400' : 'text-slate-400 hover:text-white'}`}>
              {t}
            </button>
          ))}
        </div>
                ...
                <div className="p-4">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <div className="flex items-center gap-1 text-slate-400 text-xs"><MapPin className="w-3 h-3" />{dest.country}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-emerald-400 text-sm font-semibold">₹{(dest.avgCost / 1000).toFixed(0)}K</div>
                      <div className="text-slate-500 text-xs">avg/trip</div>
                    </div>
                  </div>
                  <p className="text-slate-400 text-xs mb-3 line-clamp-2">{dest.description}</p>
            ...
            {mockActivities.map((act, i) => (
              <motion.div key={act.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
                ...
                <div className="text-right flex-shrink-0">
                  <div className="text-emerald-400 text-sm font-semibold">₹{act.cost.toLocaleString()}</div>
